...
const sampleGenders = ['Male', 'Female', 'Erkek', 'Kadın'];
const samplePhones = [
  '+90 532 111 22 33',
  '+90 533 222 33 44',
  '+90 534 333 44 55',
...
const run = async () => {
  ...
  try {
    ...
    const rows = await client.query('SELECT uid, birth_date, gender, data FROM users ORDER BY created_at ASC, uid ASC');
    let updated = 0;

    for (let i = 0; i < rows.rowCount; i += 1) {
      const row = rows.rows[i];
      const data = row.data && typeof row.data === 'object' ? { ...row.data } : {};

      const nextBirthDate = isMissing(row.birth_date || data.birthDate)
        ? sampleBirthDates[i % sampleBirthDates.length]
        : String(row.birth_date || data.birthDate);

      const nextGender = isMissing(row.gender || data.gender)
        ? sampleGenders[i % sampleGenders.length]
        : String(row.gender || data.gender);

      const nextPhone = isMissing(data.phone)
        ? samplePhones[i % samplePhones.length]
        : String(data.phone);
      const nextAvatarUri = resolveAvatarUri(nextGender, row.uid || data.email || data.displayName || data.fullName || String(i));
